...
const MainBackdrop = () => {
	...
	const keyBindings = s.app.keyboard.keyBindingContributions.values()
	const kb = keyBindings.find((k) => k.id === showCommandPalette.id)!

	return (
		<div className="flex flex-col text-center items-center justify-center text-4xl">
			<img src={logo} className="App-logo" alt="logo" />
			<p>
				Press <code>{formatKeyBinding(s.app, kb).key}</code> to open command palette.
			</p>
			<a className="text-blue-300" href="https://reactjs.org" target="help" rel="noopener noreferrer">
				Learn React
			</a>
		</div>
	)
